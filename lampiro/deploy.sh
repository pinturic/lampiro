...
do
	...
	zip -j $RESPATH/compression/lampiro-${LANG_FILE}-10.3-compression.zip $RESPATH/compression/lampiro.jad $RESPATH/compression/lampiro.jar

	ant deployedSuite -Dlang=$LANG_FILE build
	cp deployed/lampiro.ja* $RESPATH/base/
	zip -j $RESPATH/base/lampiro-${LANG_FILE}-10.3.zip $RESPATH/base/lampiro.jad $RESPATH/base/lampiro.jar

	ant deployedSuite -propertyfile eclipseme-build-TLS.properties -Dlang=$LANG_FILE build
	cp deployed/lampiro.ja* $RESPATH/TLS/
	zip -j $RESPATH/TLS/lampiro-${LANG_FILE}-10.3-TLS.zip $RESPATH/TLS/lampiro.jad $RESPATH/TLS/lampiro.jar
done
